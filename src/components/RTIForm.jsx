...
export default function RTIForm() {
  ...
  const handleAutoFill = async (e) => {
    ...
    setLoading(true);
    try {
      const result = await draftRTIRequest(problem, mockDepartments);
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 bg-white rounded-lg shadow-md mt-10">
      ...
      <form onSubmit={handleAutoFill} className="space-y-4">
        ...
        >
          {loading ? 'AI Auto-Selecting Ministry & Drafting...' : 'Auto-Fill Ministry & Generate Draft'}
        </button>
      </form>
  ...
  <div className="mt-8 pt-6 border-t border-gray-200 space-y-4">
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">Selected Ministry</label>
        <input
          type="text"
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">Public Authority</label>
        <input
          type="text"
    ...
    <div>
      <label className="block text-sm font-medium text-gray-700 mb-1">Generated Legal RTI Text</label>
      <textarea
        rows="6"
        value={draftedText}
        onChange={(e) => setDraftedText(e.target.value)}
        className="w-full p-3 bg-white border border-gray-300 rounded-md font-mono text-sm text-gray-900 focus:ring-2 focus:ring-blue-500"
      />
    </div>
        ...
        <div className="mt-8 pt-6 border-t border-gray-200 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                type="text"                
                value={selectedMinistry}
                className="w-full p-2.5 bg-gray-100 border border-gray-300 rounded-md font-medium text-gray-800"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Public Authority</label>
          ...
          <div>
            ...
            <textarea
              rows="6"              
              value={draftedText}
              className="w-full p-3 bg-gray-50 border border-gray-300 rounded-md font-mono text-sm text-gray-900"
            />
          </div>
        </div>
      )} */}
